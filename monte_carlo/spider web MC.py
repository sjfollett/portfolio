...
# print(df)
todayPrice = df["Close"][0]  # 0 for price today as starting point
print(todayPrice)
...
ax.set_facecolor("k")
plt.grid(which="major", axis="both", color="lightslategrey", linestyle="-", linewidth=".5")
plt.title(f"{ticker} Monte Carlo 10,000 Scenarios for 252 Days (2.5 million calculations)")
plt.xlabel(f"Days from today ({datetime.date(toDate)})")
plt.ylabel("Price")
plt.show()
